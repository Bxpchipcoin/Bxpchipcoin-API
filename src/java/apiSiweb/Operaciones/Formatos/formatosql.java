package apiSiweb.Operaciones.Formatos;
   ...
   public String ExecuteSQLMaster(Conexion oConn, int intIdKey) {
      ...
      if (this.getFieldInt("FS_ID") != 0) {
         if (!this.getFieldString("FS_SQL").equals("")) {
            ...
            //Reemplazamos por comodines
            if(strSQL.contains("[no_user]")){
               if(this.varSesiones != null){
                  strSQL = strSQL.replace("[no_user]", varSesiones.getIntNoUser() + "");
               }
            }
            //Reemplazamos por comodines
            if(strSQL.contains("[EmpresaDef]")){
               if(this.varSesiones != null){
                  strSQL = strSQL.replace("[EmpresaDef]", varSesiones.getIntIdEmpresa() + "");
               }
            }
            //Reemplazamos por comodines
            if(strSQL.contains("[SUCURSAL]")){
               if(this.varSesiones != null){
                  strSQL = strSQL.replace("[SUCURSAL]", varSesiones.getIntSucursalDefault() + "");
               }
            }
            //Reemplazamos por comodines
            if(strSQL.contains("[SM_MASTER]")){
               if(this.varSesiones != null){
                  strSQL = strSQL.replace("[SM_MASTER]", varSesiones.getIntSucursalMaster() + "");
               }
            }
