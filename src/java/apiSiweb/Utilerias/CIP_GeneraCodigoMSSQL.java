...
import apiSiweb.Operaciones.Formularios;
import apiSiweb.Operaciones.FormulariosDeta;
 ...
 * @author ZeusSIWEB
 */
public class CIP_GeneraCodigoMSSQL {
   ...
   public static void GeneraPantallasAuto(ArrayList<String> lstTablas, Conexion oConn) {
      ...
      while (it.hasNext()) {
         ...
         try {
            String strSql = "exec sp_columns  " + strNameTable;
            ResultSet rs = oConn.runQuery(strSql, true);
            int intOrden = -1;
            while (rs.next()) {
               String strName = new String(rs.getString("COLUMN_NAME"));
               String strType = new String(rs.getString("TYPE_NAME"));
               int intPrecision = rs.getInt("PRECISION");
               String strExtra = "";
//               System.out.println("strName:" + strName + " strType:" + strType + " intPrecision:" + intPrecision + " strExtra:" + strExtra);
               intOrden++;
               ...
               if (strType.toLowerCase().contains("identity")) {
               ...
               if (strType.contains("text")) {
                  ...
                  if (strType.toLowerCase().contains("varchar")) {
                     strTipoValorDefa = "";
                     strTipoDato = "text";
                     intAncho = intPrecision;
                  } else {
