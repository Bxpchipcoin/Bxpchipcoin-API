...
import apiSiweb.Utilerias.Sesiones;
...
public class MobilserviceResource {
   ...
   public String getXml(
      @DefaultValue("") @QueryParam("user") String user, @DefaultValue("") @QueryParam("password") String password) {
      ...
      try {
         ...
         if (!user.isEmpty() && !password.isEmpty()
            && !user.contains("'") && !password.contains("'")) {
            action.setBolEsBackOffice(false);
            action.authentication_user(oConn, user, password, servletRequest);
         } else {
