...
import apiSiweb.Operaciones.usuarios_accesos;
...
//         }
         ...
         if (!this.bolSoloCliente) {
            ...
            if(oConn.getStrDriverName().contains("jTDS")){
               SQL_QUERY = "select * from usuarios where usuario = '" + usuario.toUpperCase() + "'";
            }
...
//                  }
               ...
               if (!bolErrorBloqued) {
                  // <editor-fold defaultstate="collapsed" desc="Validamos si correspone el password y esta activo">
                  if (password.toUpperCase().equals(rs.getString("CT_PASSWORD").toUpperCase() ) /*&& rs.getInt("CT_ACTIVO") == 1*/) {
